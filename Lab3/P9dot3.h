...
namespace P9dot3 {
	...
	// Custom exception to be used below in the map class
	...
	class MyMap
	{
		typedef std::vector<MapElement>::iterator Iterator;
	public:
		MyMap();
		~MyMap();
		...
		Iterator put(int key, int value); // If no key is present, insert key/value pair. Otherwise update the value at the given key.
